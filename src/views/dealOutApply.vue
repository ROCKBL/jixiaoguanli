<template>
	<div class="dealOutApply">
		<van-nav-bar title="外出申请"  left-arrow class="pageNavNative" @click-left="onClickLeft" />
			...
			<div class="messageUnit" v-for="item in messageList" v-if="item.empId!=userInfo.id">
				...
				<div @click="seeDetail(item)">
					<div class="messageUnitTitle">
						...
						<div class="messageUnitStatus" :class="statusClass(item.approvalStatus)">{{ statusLabel(item.approvalStatus) }}</div>
					</div>
				...
				<div class="messageUnitContent" v-show="item.active" :class="item.active?'messageUnitContentActive':''">
						...
						<div class="applyForOutTable">
							...
							<el-row class="applyForOutBodyRow">
							  	...
							  	<el-col :span="4" class="applyForOutCell">
							  		<el-input v-model="item.empName" class="value" disabled></el-input>
							  	</el-col>
							  	<el-col :span="4" class="applyForOutCell label">部门</el-col>
							  	<el-col :span="4" class="applyForOutCell">
							  		<el-input v-model="item.departName" class="value" disabled></el-input>
							  	</el-col>
							  	<el-col :span="4" class="applyForOutCell label">岗位职务</el-col>
							  	<el-col :span="4" class="applyForOutCell">
							  		<el-input v-model="item.stationName" class="value" disabled></el-input>
							  	</el-col>
							</el-row>
							<el-row class="applyForOutBodyRow">
							  	<el-col :span="4" class="applyForOutCell label">外出时间</el-col>
							  	<el-col :span="4" class="applyForOutCell">
							  		<el-input v-model="item.outTime" class="value" disabled></el-input>
							  	</el-col>
							  	<el-col :span="4" class="applyForOutCell label">外出地点</el-col>
							  	<el-col :span="4" class="applyForOutCell">
							  		<el-input v-model="item.outAddress" class="value" disabled></el-input>
							  	</el-col>
							  	<el-col :span="4" class="applyForOutCell label">审批状态</el-col>
							  	<el-col :span="4" class="applyForOutCell newValue" :class="statusClass(item.approvalStatus)" >{{ statusLabel(item.approvalStatus) }}</el-col>
							</el-row>
							...
							<el-row class="applyForOutBodyRow applyForOutBodyRowHigh">
							  	...
							  	<el-col :span="20" class="applyForOutCell">
							  		<el-input v-model="item.outDescribe" class="value" type="textarea" disabled></el-input>
							  	</el-col>
							</el-row>
							<el-row class="applyForOutBodyRow applyForOutBodyRowHigh">
							  	<el-col :span="4" class="applyForOutCell label">外出期间工作安排</el-col>
							  	<el-col :span="20" class="applyForOutCell">
							  		<el-input v-model="item.workScheduling" class="value" type="textarea" disabled></el-input>
							  	</el-col>
							</el-row>

							<el-row class="applyForOutBodyRow">
							  	<el-col :span="4" class="applyForOutCell label">审批人</el-col>
							  	<el-col :span="4" class="applyForOutCell newValue">{{ item.approvalPerson }}</el-col>
							  	<el-col :span="4" class="applyForOutCell label">审批意见</el-col>
							  	<el-col :span="4" class="applyForOutCell newValue">{{ item.approvalOpinion }}</el-col>
							  	<el-col :span="4" class="applyForOutCell label">审批时间</el-col>
							  	<el-col :span="4" class="applyForOutCell newValue">{{ item.approvalTime }}</el-col>
							</el-row>
						</div>
					</div>

					<div class="btnsRow" v-if="item.approvalStatus=='审核中'&&item.auth==1">
					...
					<div class="btnsRow" v-if="item.approvalStatus=='审核通过'&&(item.auth==0||item.auth==3)&&!item.managerView">
						<van-button @click="know(item)" size="small" class="agreeBtn" type="info">知晓</van-button>
					</div>
	...
	methods:{
		know(item){
			...
			Dialog.confirm({
			    ...
			    // on confirm
			    var url=that.baseUrl+"/outApply/edit";
				var data={
					id:item.id,
					managerView:true
				}
        ...
        getData(){
        	var that=this;
			var url=this.baseUrl+"/outApply/list";
