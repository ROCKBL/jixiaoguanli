<template>
	<div class="dealOutApply">
		<van-nav-bar title="提案改善建议"  left-arrow class="pageNavNative" @click-left="onClickLeft" />
			...
			<div class="messageUnit" v-for="item in messageList" v-if="item.empId!=userInfo.id">
				<div @click="seeDetail(item)">
					<div class="messageUnitTitle">
						...
						<div class="messageUnitStatus" :class="statusClass(leaderCheckLabel(item.leaderCheckReult))">{{ statusLabel(leaderCheckLabel(item.leaderCheckReult)) }}</div>
					</div>
				...
				<div class="messageUnitContent" v-show="item.active" :class="item.active?'messageUnitContentActive':''">
					...
					<div class="btnsRow" v-if="item.leaderCheckReult==0||item.leaderCheckReult==null">
	...
	methods:{
		...
		agree(item){
			...
			Dialog.confirm({
			    ...
			    // on confirm
			    var url=that.baseUrl+"/proposal/addLeaderResult";
				var params={
					id:item.id,
					status:2
				}
        ...
        getData(){
        	var that=this;
			var url=this.baseUrl+"/proposal/list";
